...
- **Unknown-model context window shown inconsistently (32 768 vs 200 000)**: when a model wasn't in `model_ctx_registry.json` (and its provider section had no `default`), the Profiles → Behavior dialog showed a "safe default" of **32 768** while the active-profile tooltip and the live CTX bar showed **200 000** (via `_fallback_ctx_window`). Unified the fallback on **32 768** across all display paths (`Controller._fallback_ctx_window` and the tooltip's defensive branch, matching the Behavior dialog's existing `_CTX_SAFE_DEFAULT`) so the same value appears everywhere. 32 768 is the conservative choice — under-estimating an unknown window is safe, whereas over-estimating risks overflowing the model's real context. Ollama keeps its own smaller local default (4 k); providers with an explicit registry value (e.g. Anthropic 200 000) are unaffected
