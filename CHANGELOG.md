...
- **psfix — appends the failed ps\* tool's own `-h` help**: when the command being fixed is a PurrSh3ll tool (`psrag`, `pscmd`, `pstldr`, `psview`, `psreport`, `psai`/`psask`/`pschat`), psfix now runs that tool's `-h` and adds its usage text to the prompt in all three modes (fix / explain / analyze), so the model knows the tool's real syntax and can propose a valid invocation instead of guessing flags. The tool's help handler is an early `sys.exit(0)` (no model/network work), the call is capped at 4 s with a 1 500-char cap on the injected text, and it is fully fail-safe — a non-ps\* command, a tool whose help lives only in its zsh wrapper (`psopen`, `pshelp`), or any error simply injects nothing. Commands are mapped to their authoritative module only, so a wrong tool's help is never surfaced
