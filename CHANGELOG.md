...
- **psreport `-N/--nano`**: new nano mode for 4K context models — section-by-section report generation without a synthesis call; two-phase pipeline: (1) same per-phase extraction as `--chunked` (one JSON call per active phase), (2) seven sequential section calls (`scope` → `assets` → `vulnerabilities` → `credentials` → `timeline` → `recommendations` → `executive_summary`); each section prompt contains compact phase JSON (~1 200 chars max) + abbreviated intel header + rolling carry-forward context from the last 2 sections (~400 chars each) for cross-section coherence; Executive Summary generated last but placed first in the assembled report (standard pentest format); app assembles the final Markdown or HTML report from section outputs without a synthesis call; `_resolve_placeholders()` runs on assembled output; pre-flight confirm shows API call count (`N_phases + 7 sections`); prompt budget per section ≈ 600–1 000 tokens → fits 4K model with ~1 000+ tokens for output
