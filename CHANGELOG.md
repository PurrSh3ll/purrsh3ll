...
- **AI Settings → AI/LLM**: new checkbox "Clear pschat history on exit" — when enabled, all `appdata/chat_sessions/*.json` files are deleted on application close; default off
...
- **psai chat**: `num_ctx` option passed to Ollama API when `context_tokens` is set in the active profile
- **psai ask / chat**: Fast answers — appends brevity instruction to query (ask) or prepends system message (chat) when "Fast answers" is checked and no custom params are set
...
- **Themes**: added Sakura Protocol and Tropical Brazil themes; replaced Matrix theme with MS-DOS theme
...
- **CSV table viewer**: table colors did not match current theme — `QTableView` selectors added to `qss_QTable`, stylesheet applied to `table_view` widget and registered in `tracked_tables` for live re-theming
