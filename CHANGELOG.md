...
- **Edit → Erase all data…**: new dialog to permanently delete the data PurrSh3ll collects, with a checkbox per category so you choose exactly what goes. Engagement data that simply accumulates is pre-checked — command history database (`terminal_history.db`, incl. psnmap/.purr runs and extracted findings/targets/ports), script notes, side-panel notepad, AI chat sessions, RAG knowledge-base index, analyzed images/screenshots and application logs; user-authored or sensitive items are left unchecked by default — saved system variables, script generated help, snippets, API keys (OS keyring + `api_keys.json`), and an optional **stop & remove ALL Docker containers** (incl. Open WebUI and WebMap; prompts for the sudo password, reusing the session's cached one, and runs `docker rm -f` with args passed safely — never via the shell). Guarded by a **type-`ERASE`-to-confirm** field plus a second confirmation listing exactly what will be deleted, then a per-category result report. The history DB handle is closed before deletion (and re-opened empty), cleared variables reload live, and each category is isolated so one failure never aborts the rest. Backed by a single registry in `core/data_wipe.py` so the wipe stays complete as new stores are added. It is an application-level erase (files/records removed), not a forensic disk wipe — the dialog says so
- **psfix — appends the failed ps\* tool's own `-h` help**: when the command being fixed is a PurrSh3ll tool, psfix now adds that tool's own usage text to the prompt in all three modes (fix / explain / analyze), so the model knows the tool's real syntax and can propose a valid invocation instead of guessing flags. Help is taken from each tool's **authoritative** source: python-module tools (`psrag`, `pscmd`, `pstldr`, `psview`, `psreport`, `psai`) via `python <module> -h` (an early `sys.exit(0)`, no model/network work, 4 s timeout), and zsh-function tools (`psask`, `pschat`, `psopen`) by extracting the help heredoc from their `.zsh` wrapper — because `psask`/`pschat` forward to `psai ask/chat` so `psai.py -h` would only show generic psai help, not their real per-command options. The injected text is capped at 1 500 chars and the whole step is fail-safe: a non-ps\* command, a meta tool with no static help (`pshelp`), or any error injects nothing, and each command maps only to its own help so a different tool's usage is never surfaced
...
- **Script loader — full-screen code view**: clicking the **code** tab in the `.py`/`.purr` loader now hides all the surrounding chrome (Install button, info bar, the `help`/`readme`/`notes`/`code` tab buttons and the entire bottom control panel — interpreter, run options, terminal input) and gives the code editor the full height of the panel, like opening the file normally. A **← Back** button is shown to return to the previous view. Makes reading/analyzing code far roomier; the chrome is wrapped in header/footer containers toggled as a group, so nothing else changes
...
- **Script loader "docs" tab (.py/.purr)**: removed the docstring-extraction tab and all its logic — `docs` button/field, `get_docs_path`, `update_docs` (AST docstring parser), `docs_text`, the `scripts_docs/` folder (controller paths, `SCRIPT_DATA_FOLDERS`) and the Erase-all-data category (`script_extras` now covers `scripts_help/` only). Docstrings are already visible in the `code` view, so the separate tab was redundant. The `help`/`readme`/`notes`/`code` tabs are unchanged (the `"docs"` QSS objectName used for text-area styling is retained).
- **psnmap `.purr` — legacy `history` key stripped on save**: with per-scan history removed, the psnmap options dialog now drops any leftover `history` array from the `.purr` JSON whenever it is written (port/profile changes), via a single `_write_purr` helper — existing files are cleaned the next time they are saved.
- **Per-script execution history & favorites (redundant with pshistory)**: removed the per-script "history" and "favorites" tabs from the `.py`/`.purr` script loader and the "history" tab from the psnmap loader — for embedded-terminal runs these commands are already captured globally in `terminal_history.db`/`pshistory`, so the per-script CSVs (and the psnmap `.purr` `history` JSON key) duplicated that data while adding UI and on-disk surface (empty in practice). Deleted `gui/panels/history_script_wdg.py` and `gui/panels/favorites_script_wdg.py`; dropped `get_history_path`/`get_favorite_path`/`update_history`, the history CSV write in `_execute_command`, and psnmap's `_append_history`/`_refresh_history_table`/`clear_history`; removed the `scripts_history/` and `scripts_favorities/` folders (controller paths, `SCRIPT_DATA_FOLDERS`, and the Erase-all-data categories). **Script notes are kept** (a distinct, non-redundant per-script annotation). Script loaders now expose `code`/`run` plus `help`/`docs`/`readme`/`notes`
