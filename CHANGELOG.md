...
- **Video file viewer**: video files (mp4, mkv, mov, avi, wmv, flv, webm, m4v, 3gp, ogv, mpeg4) now open in a dedicated in-app viewer with playback controls (play/pause/stop, seek bar, volume), file info panel (duration, resolution, FPS, codec, bitrate, size, permissions), metadata panel powered by `exiftool` (GPS coordinates marked with 📍, encoder, creation date, device make/model, all OSINT-relevant fields shown first), and an integrity section with MD5 + SHA256 hashes (copyable) plus an anomaly check (size/duration ratio — flags potential steganography or re-encoding); "Open in system player" button available in title bar; powered by `QMediaPlayer` + `QVideoWidget` (PyQt6 QtMultimedia) and `exiftool`
