...
PurrSh3ll records all terminal commands in a SQLite database (`appdata/logs/terminal_history.db`).
Use the built-in `pshistory` command to query it before any task:

```bash
pshistory -n 30              # last 30 commands with output
pshistory -t recon           # all recon-phase commands
pshistory -t exploit         # all exploit-phase commands
pshistory --targets          # discovered targets and open ports
pshistory --findings         # extracted credentials, hashes, CVEs, flags, users
pshistory -q nmap            # search commands and outputs matching "nmap"
pshistory --stats            # summary: total commands, phases used, findings count
```
...
After solving the box, generate a writeup using `psreport`. This command reads the terminal history database (discovered services, credentials, flags, phase coverage, commands run) and produces a structured AI-generated session report.

### Generate the writeup

```bash
# Standard writeup — uses Max terminal turns setting
psreport

# Deep writeup — includes all commands from the session
psreport --full

# Filter by target IP
psreport -t <target_ip>

# Save directly to file
psreport > writeup_<machinename>.md
```

> If `goal.md` was not used in this session, `psreport` still works — it reads directly from the terminal history database, so any commands run in this terminal are already captured.

### Optional: convert to other formats

```bash
pandoc writeup_<machinename>.md -o writeup_<machinename>.pdf
pandoc writeup_<machinename>.md -o writeup_<machinename>.docx
```

### Writeup structure (generated by psreport)

The AI generates a report covering:
- Summary of the attack path
- Enumeration findings (open ports, services, versions)
- Credentials and hashes discovered
