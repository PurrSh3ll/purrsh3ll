...
_EXPLOIT_STEPS = {
    ...
    "snmp": [
        "Guess community strings (onesixtyone / snmpwalk: public, private, community)",
        "snmpwalk the full tree — hostname, users, processes, routes, ARP, listening ports",
        "Extended MIBs: running processes w/ args (creds!), installed software, local users",
        "SNMP v3 → enumerate/brute users; writable community → tamper config",
    ],
    "ipmi": [
    ...
    "smtp": [
        "Banner & exact version → searchsploit",
        "Username enumeration via VRFY / EXPN / RCPT TO (smtp-user-enum)",
        "Open-relay test (send mail to an external domain)",
        "Auth creds (reused); command injection / known CVE (e.g. Exim, Postfix Shellshock)",
    ],
    "mail2": [
    ...
    "ssh": [
        "Banner → exact version; searchsploit (libssh auth bypass, older OpenSSH)",
        "Enumerate valid users (CVE-2018-15473 timing) and list supported auth methods",
        "Reused / known creds & private keys found elsewhere; careful, targeted spray",
        "Weak host algorithms / terrapin; check for shell restrictions to bypass (rbash)",
    ],
    "squid": [
