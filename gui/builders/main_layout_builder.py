    ...
    def create_welcome_text():
        ...
        def _apply_image(path, label):
            ...
            if path.lower().endswith(".gif"):
                ...
                s = label.size()
                if nat.isValid() and nat.width() > 0 and s.width() > 0:
                    m.setScaledSize(nat.scaled(s, Qt.AspectRatioMode.KeepAspectRatio))
                label.setMovie(m)
                m.start()
                _current_movie[0] = m
        ...
        class _GifLabel(QLabel):
            ...
            def paintEvent(self_, event):
                if self_._orig_pixmap and not self_._orig_pixmap.isNull():
                    from PyQt6.QtGui import QPainter
                    s = self_.size()
                    scaled = self_._orig_pixmap.scaled(
                        s,
    ...
    def create_active_profile_combo():
        ...
        def _resolve_ctx(p):
            ...
            try:
                ...
                # ollama + local runtimes 4k / cloud 200k
                if provider == "ollama" or provider in {"llamacpp", "lmstudio", "jan", "koboldcpp"}:
                    fb = 4096
                else:
                    fb = 200_000
            return fb, True
