...
class Html_file(ChunkedFileLoader):
    ...
    def _on_finished(self, path, content):
        ...
        if isinstance(content, str) and content.startswith("__ERROR__:"):
            ...
            open_browser_btn.setToolTip("Preview this page in your default web browser")

            def _open_in_browser():
                try:
                    msg = QMessageBox(parent= self.parent.widgets['execution_tabs'])
                    self.qss_QMessagebox_style = self._controller.messagebox_stylesheet
                    msg.setStyleSheet(self.qss_QMessagebox_style)
                    msg.setWindowTitle("Open in Browser")
                    msg.setText("Do you want to open this file in your default web browser?")
                    ...
                    if user_response == QMessageBox.StandardButton.Yes:
                        ...
                        confirm.setWindowTitle("Information")
                        confirm.setText("Opening the file in your default web browser...")
                        confirm.setIcon(QMessageBox.Icon.Information)
                        confirm.exec()

                except Exception as e:
                    error_box = QMessageBox(parent= self.parent.widgets['execution_tabs'])
                    error_box.setStyleSheet(self.qss_QMessagebox_style)
                    error_box.setWindowTitle("Error")
                    error_box.setText(f"Failed to open file:\n{e}")
                    error_box.setIcon(QMessageBox.Icon.Critical)
                    error_box.exec()

            open_browser_btn.clicked.connect(_open_in_browser)
            control_bar_layout.addWidget(open_browser_btn)
            ...
            def _add_info_labels():
                ...
                unsupported_info_label = CustomUnsupportedLabel(parent=control_bar_widget)
                unsupported_info_label.setText("Syntax Highlighting Disabled for files above 1000 lines")
                unsupported_info_label.setObjectName("unsupported_info_label")
                unsupported_info_label.setEnabled(False)
                unsupported_info_label.setVisible(False)
                layout.addWidget(unsupported_info_label)

            self.text_widget = _create_text_widget()

            self.content_area, content_area_layout = _create_content_area()
            splitter = _create_splitter(self.content_area)

            self.left_container = _wrap_in_container(self.text_widget, self.content_area)
            splitter.addWidget(self.left_container)

            preview_widget = None
            if getattr(self._controller, "lightweight_web_browser", False) is True:
                preview_widget = QTextEdit(self.content_area)
                ...
                if 'content' in locals() and content:
                    preview_widget.setHtml(content)
                else:
                    preview_widget.setHtml(self.text_widget.toPlainText())

                self.text_widget.textChanged.connect(lambda: preview_widget.setHtml(self.text_widget.toPlainText()))

            else:
                self.preview = WebPreview(parent=self.content_area)
                ...
                preview_widget.page().settings().setAttribute(
                ...
                if os.path.exists(file_path):
                    preview_widget.load(QUrl.fromLocalFile(file_path))
                else:
                    html = self.text_widget.toPlainText() if hasattr(self, "text_widget") else (
                        content if 'content' in locals() else "")
                    preview_widget.setHtml(html, base_qurl)

                self._preview_timer = QTimer(preview_widget)
                self._preview_timer.setSingleShot(True)
                self._preview_timer.setInterval(500)

                def _update_preview():
                    try:
                        html = self.text_widget.toPlainText()
                    except Exception:
                        html = ""
                    current_scheme = preview_widget.url().scheme()
                    if current_scheme == "file":
                        return
                    preview_widget.setHtml(html, base_qurl)

                self._preview_timer.timeout.connect(_update_preview)
                self.text_widget.textChanged.connect(lambda: self._preview_timer.start())
            ...
            content_area_layout.addWidget(splitter)
            ...
            if self._controller.web_browser_3d_info:
                info_bar = QFrame(parent=control_bar_widget)
                ...
                info_label = QLabel(
                    "If you are using a <b>virtual machine</b>, please note that for proper web browser performance "
                    "it may be necessary to <b>disable 3D graphics acceleration</b> or set settings to use <b>lightweight web browser</b>"
                )
                ...
                close_btn = QPushButton("✕", parent = info_bar)
                ...
                def hide_info_bar():
                    info_bar.hide()
                    self._controller.web_browser_3d_info = False
            ...
            try:
                if hasattr(self, "_controller") and getattr(self._controller, "text_loaders", None) is not None:
                    try:
                        ...
                        pass
                if hasattr(self, "_controller") and getattr(self._controller, "text_highlighters", None) is not None:
                    try:
                        if getattr(self, "syntax_highlighter", None) in self._controller.text_highlighters:
                            self._controller.text_highlighters.remove(self.syntax_highlighter)
                    except Exception:
                        pass
            except Exception:
                pass
