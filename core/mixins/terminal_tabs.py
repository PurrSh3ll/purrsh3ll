...
class TerminalTabsMixin:
    ...
    def _create_split_terminal(self, wrapper_widget):
        ...
        def _on_ctx(pos, t=term, w=wrapper_widget):
            ...
            act_zi.triggered.connect(lambda checked=False, tt=t: (tt.setFocus(), tt.zoom(1)) if hasattr(tt, "zoom") else None)
            act_zo = QAction("Zoom Out", menu)
            act_zo.triggered.connect(lambda checked=False, tt=t: (tt.setFocus(), tt.zoom(-1)) if hasattr(tt, "zoom") else None)
